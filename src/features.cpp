                              ...
                              int _maxSampleCount, Size _winSize )
{
    ...
    winSize = _winSize;
    numFeatures = 0;
    cls.create( (int)_maxSampleCount, 1, CV_32FC1 );	//创建一个numPos + numNeg行1列，一通道的32位float型的Mat类对象
    generateFeatures();	//CvFeatureEvaluator类中的generateFeatures函数尚为纯虚函数，没有实际意义
}
