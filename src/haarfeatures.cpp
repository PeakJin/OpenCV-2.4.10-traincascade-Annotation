...
//用于根据DOS命令来选择Harr特征的mode
bool CvHaarFeatureParams::scanAttr( const string prmName, const string val)
{
    if ( !CvFeatureParams::scanAttr( prmName, val ) )	//CvFeatureParams公共继承自CvParams
    {
        if( !prmName.compare("-mode") )
        {
            mode = !val.compare( CC_MODE_CORE ) ? CORE :		//#define CC_MODE_CORE   "CORE" 
                   !val.compare( CC_MODE_ALL ) ? ALL :			//enum { BASIC = 0, CORE = 1, ALL = 2 };
